...
EXTERN int luasteam_Apps_GetDlcDownloadProgress(lua_State *L) {
	...
	bool __ret = SteamApps()->GetDlcDownloadProgress(nAppID, &punBytesDownloaded, &punBytesTotal);
	lua_pushboolean(L, __ret);
...
EXTERN int luasteam_Apps_BIsTimedTrial(lua_State *L) {
	...
	bool __ret = SteamApps()->BIsTimedTrial(&punSecondsAllowed, &punSecondsPlayed);
	lua_pushboolean(L, __ret);
...
EXTERN int luasteam_Apps_GetNumBetas(lua_State *L) {
	int pnAvailable;
	int pnPrivate;
	int __ret = SteamApps()->GetNumBetas(&pnAvailable, &pnPrivate);
	lua_pushinteger(L, __ret);
